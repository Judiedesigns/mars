...
export default function ProjectReel() {
  ...
  const duplicatedImages = [...reelImages, ...reelImages];
  ...
  return (
    <section className="pb-[72px] lg:pb-[96px] animate-fade-in stagger-6 overflow-hidden">
          ...
          >
            {duplicatedImages.map((image, index) => {
              ...
              return (
                <div
                  key={`${image.id}-${index}`}
                  className="flex-shrink-0 reel-item"
                ...
                >
                  <div className="bg-[#F2F2F2] w-full p-[12px] rounded-[12px] h-[280px] sm:h-[320px] lg:h-[360px] transition-transform duration-500 ease-out hover:scale-[1.03]">
                    <div className="bg-[#efefef] w-full h-full overflow-hidden rounded-[8px] relative">
                      {/* Loading skeleton */}
                      {!isLoaded && (
